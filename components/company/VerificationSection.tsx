...
export default function VerificationSection({ company, onUpdate }: Props) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 1. Upload File
      const downloadUrl = await uploadUserFile(currentUser, file, `verification/CUI_${Date.now()}_${file.name}`);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Verificare Companie</h3>
          <p className="text-sm text-gray-600">
            Pentru a câștiga încrederea clienților, încarcă certificatul de înregistrare (CUI).
          </p>
        </div>
      ...
      <div className="mt-6 border-t border-gray-100 pt-6">
         {status === 'verified' ? (
           <div className="flex items-center gap-3 rounded-xl bg-emerald-50 p-4">
             ...
             <div>
               <p className="font-semibold text-emerald-900">Felicitări! Contul tău este verificat.</p>
           ...
           <div className="flex items-center gap-3 rounded-xl bg-amber-50 p-4">
             ...
             <div>
               <p className="font-semibold text-amber-900">Document trimis spre verificare.</p>
               <p className="text-sm text-amber-700">Un administrator va analiza documentul în scurt timp.</p>
             </div>
           </div>
         ) : (
           <div className="space-y-4">
             {status === 'rejected' && company.rejectionReason && (
               <div className="mb-4 flex items-start gap-3 rounded-xl bg-rose-50 p-4">
                 ...
                 <div>
                   <p className="font-semibold text-rose-900">Documentul a fost respins</p>
                   <p className="text-sm text-rose-700">Motiv: {company.rejectionReason}</p>
                 </div>
               </div>
             )}
               ...
               <label htmlFor="doc-upload" className="cursor-pointer">
                 {uploading ? (
                    <div className="flex flex-col items-center">
                       ...
                       <span className="text-sm font-medium text-slate-600">Se încarcă...</span>
                    </div>
                 ) : (
                    <div className="flex flex-col items-center">
                       <UploadIcon className="mb-3 h-10 w-10 text-slate-400" />
                       <span className="text-base font-semibold text-slate-700">Încarcă Certificatul (CUI)</span>
                       <span className="mt-1 text-xs text-slate-500">PDF, JPG sau PNG (max 5MB)</span>
