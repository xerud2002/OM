...
export default function UrgencyBanner({ city = "București", className = "" }: UrgencyBannerProps) {
  ...
  return (
    <div className={`relative z-20 flex flex-col items-center gap-2 ${className}`}>
      ...
      <div className="flex flex-col items-center gap-3 rounded-full border border-white/40 bg-white/80 py-2 pl-3 pr-6 shadow-xl shadow-emerald-900/5 backdrop-blur-xl sm:flex-row sm:gap-6 sm:py-2.5">
        
        {/* Left: Active Users with Avatars */}
        <div className="flex items-center gap-3 border-b border-gray-100 pb-2 sm:border-b-0 sm:border-r sm:pb-0 sm:pr-6">
           <div className="flex -space-x-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className={`h-8 w-8 rounded-full border-2 border-white bg-gray-200 flex items-center justify-center overflow-hidden`}>
                 <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i + activeUsers}`} alt="User" className="h-full w-full" />
              </div>
            ))}
           ...
           <div className="flex flex-col">
             ...
             <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wide">Utilizatori activi</span>
           </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm">
             <p className="font-bold text-slate-800 leading-none">{requestCount} cereri azi</p>
